...
> **先读总揽**：[Transformer全链路总揽与环节拆解.md](./transformer/Transformer全链路总揽与环节拆解.md) —— 宏观主流程 + 环节导航总表 + 结构全景图（附 AIGC 扩展）
> 每个环节一份详解文档（干什么 / 怎么干 / 方法对比 / 优缺点 / 面试题）：
> - 入口三站：环节 01 Tokenizer → [环节01-Tokenizer分词详解.md](./transformer/环节01-Tokenizer分词详解.md) ｜ 环节 02 Embedding → [环节02-Embedding查表详解.md](./transformer/环节02-Embedding查表详解.md) ｜ 环节 03 位置编码/RoPE（零基础一步步版）→ [环节03-位置编码详解.md](./transformer/环节03-位置编码详解.md)
> - 核心机制：环节 04 Attention（含数值手算 + 可运行代码）→ [环节04-Attention注意力详解.md](./transformer/环节04-Attention注意力详解.md) ｜ 环节 05 FFN·MoE → [环节05-FFN激活与MoE详解.md](./transformer/环节05-FFN激活与MoE详解.md) ｜ 环节 06 残差·归一化 → [环节06-残差连接与归一化详解.md](./transformer/环节06-残差连接与归一化详解.md)
> - 组装与目标：环节 07 架构与 RNN/LSTM 演进 → [环节07-Block堆叠与整体架构详解.md](./transformer/环节07-Block堆叠与整体架构详解.md) ｜ 环节 08 输出头与训练目标（与环节 04 数值走查连体）→ [环节08-输出头与训练目标详解.md](./transformer/环节08-输出头与训练目标详解.md)
> - 前史与纵深：RNN/LSTM 本体全体系（BPTT 数值手推 / LSTM·GRU / 双向·Seq2Seq / 现代类 RNN 谱系）→ [RNN知识整理.md](./rnn/RNN知识整理.md)
> - 生命周期：环节 09 训练管线 → [环节09-训练管线详解.md](./transformer/环节09-训练管线详解.md) ｜ 环节 10 推理解码与 KV Cache → [环节10-推理解码与KV缓存详解.md](./transformer/环节10-推理解码与KV缓存详解.md) ｜ 环节 11 服务化与推理引擎 → [环节11-服务化与推理引擎详解.md](./transformer/环节11-服务化与推理引擎详解.md)
...
- **RNN/LSTM 前史（本体纵深）**：串行 + 有损压缩 vs 注意力并行 + 无损可见；BPTT 梯度消失是分水岭；现代"类 RNN"（线性注意力 / RWKV / Mamba）借"状态复用"回归 —— 详见 [RNN知识整理.md](./rnn/RNN知识整理.md)
...
- **DeepSeek**：V3（MoE 基座）、R1（推理模型，GRPO 强化学习）、V3.2 / R1 蒸馏版、V4 / V4-Flash
  - 关键技术：MLA 注意力、DeepSeekMoE（细粒度专家 + 共享专家）、无辅助损失负载均衡
  - 注：V4-Flash 官方以 **DeepSeek Harness 极简模式**作为测试框架（模型与 Agent 框架协同演进，见 2.8 / 2.10）
  - 训练报告阅读整理（`../deepseek/` 目录，案例纵深）：[V3 训练报告详解](../deepseek/DeepSeek-V3训练报告详解.md)（14.8T token / FP8 / DualPipe / $5.6M 总账）｜ [R1 训练报告详解](../deepseek/DeepSeek-R1训练报告详解.md)（GRPO 四阶段 / 蒸馏，已刊 Nature）
...
- **三阶段范式**：Pre-training（预训练）→ SFT（监督微调）→ 对齐（RLHF / DPO / GRPO）
- **预训练**：数据清洗配比、Scaling Law（参数/数据/算力的关系）、涌现能力 —— 详解见 [环节09-训练管线详解.md](./transformer/环节09-训练管线详解.md)
...
- **核心优化**：
  ...
  - **投机解码 / 前缀缓存**：降延迟与成本
- **推理引擎**：vLLM、SGLang、TensorRT-LLM、Ollama（本地）、LMDeploy —— 详解见 [环节11-服务化与推理引擎详解.md](./transformer/环节11-服务化与推理引擎详解.md)
...
- **视觉**：ViT、CLIP（图文对齐，机制详解见 [多模态理解与统一模型详解.md](./aigc/多模态理解与统一模型详解.md)）、视觉理解（OCR/图表/截图问答）
- **语音**：ASR（语音识别，Whisper 系）、TTS（语音合成）、端到端语音对话
- **生成**：文生图（Diffusion / DiT）、文生视频
- **AIGC 全谱系与工程化**：文/图/音/视频/3D/代码生成范式（自回归 vs 扩散）、审核/版权/评测 —— 库入口 [AIGC总揽与多模态地图.md](./aigc/AIGC总揽与多模态地图.md)：图像扩散 [图像扩散模型详解.md](./aigc/图像扩散模型详解.md) ｜ 视频 [视频生成详解.md](./aigc/视频生成详解.md) ｜ 音频语音 [音频与语音详解.md](./aigc/音频与语音详解.md) ｜ 多模态统一 [多模态理解与统一模型详解.md](./aigc/多模态理解与统一模型详解.md)
...
- **其他范式**：Plan-and-Execute（先规划再执行）、Reflexion（反思重试）、Tree-of-Thought（多路径搜索）
  - 推理范式全家桶详解（CoT / Self-Consistency / ToT / GoT / Reflexion / ReAct，含成本权衡与工程选型）见 something/[思维链与推理范式详解.md](../something/思维链与推理范式详解.md)

### 2.2 提示工程（Prompt）

- System Prompt 设计、Few-shot、CoT、指令分层与结构化（思维链原理与变体详解见 something/[思维链与推理范式详解.md](../something/思维链与推理范式详解.md)）
...
- **协议分层**：JSON-RPC 2.0 消息 + 生命周期（initialize / notifications/initialized）+ 三大能力（tools / resources / prompts）
- **接口清单**：
  - 生命周期：`initialize`（版本协商 + 能力声明）、`notifications/initialized`
  - Tools：`tools/list`、`tools/call`、`notifications/tools/list_changed`
  - Resources：`resources/list`、`resources/templates/list`、`resources/read`、`notifications/resources/updated`
  - Prompts：`prompts/list`、`prompts/get`、`notifications/prompts/list_changed`
  - 通用：`ping`、`logging/setLevel`、`completion/complete`、`sampling/createMessage`（服务端反向调模型）
- **错误码**：-32700 解析错误 / -32601 方法未找到 / -32602 无效参数 / -32603 内部错误 / -32002 资源未找到
- **关键约束**：initialize 之前不能发其他请求；capabilities 声明驱动客户端行为；日志必须走 stderr（stdio 传输下不能污染协议流）
...
- **评估（Eval）**：离线数据集 + 指标（忠实度、相关性、召回率）、在线质量回归 → Agent 上线最大门槛（详解见 something/[Agent评测详解.md](../something/Agent评测详解.md)）
...
> Code Agent 这类编程智能体的完整拆解（验证器闭环 / code retrieval / 安全 / 评估）见 something/[CodeAgent详解.md](../something/CodeAgent详解.md)
...
- **理论基础**：CAP/BASE（详解见 something/[CAP与BASE.md](../something/CAP与BASE.md)）、一致性模型、FLP、拜占庭（了解）
