...
- **统一骨架**：文本、[多模态](../multimodal/) token、甚至部分 [扩散](../diffusion/) 骨干都往这套块上靠。
...
LLM 服务里的 **KV cache**：已经算过的 K、V 存下来，新 token 只算自己的 Q 再和历史 K 做点积。所以「接着聊」比「把整段对话当新上下文重算」便宜，直到窗口或显存顶满。这正是 [Context Engineering](../context-engineering/) 要管窗口的原因。
...
| [Speculative Decoding](../speculative-decoding/) | 加速的是自回归逐步解码，不是换骨架 |
| [Context Engineering](../context-engineering/) | 窗口 = 注意力能看见的范围；超了等于没看见 |
...
2. **窗口当物理定律**：prompt 塞不进窗口，不是「模型笨」，是注意力根本没看见。超长材料走 [RAG](../rag/) / 摘要，而不是祈祷。
...
- 对比：[moe](../moe/)、[multimodal](../multimodal/)、[context-engineering](../context-engineering/)
