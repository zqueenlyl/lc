...
安全是第一设计约束：模型一旦能点「转账」「删除生产」，等于拥有操作者会话。必须和 [Guardrails](../guardrails/) 绑死。
...
- **端到端验收**：自己走一遍用户路径，给 [Eval](../eval/) 当真实环境断言。
...
| [Multimodal](../multimodal/) | 像素路线依赖多模态感知 |
| [Eval](../eval/) | 任务完成率、步数、非法动作率是核心指标 |
...
5. 和 [Voice](../voice-realtime/) 组合时，动作确认要用语音回读关键字段。
...
- 对比：[mcp](../mcp/)、[multimodal](../multimodal/)、[guardrails](../guardrails/)
