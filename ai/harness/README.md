...
| [Guardrails](../guardrails/) | 命令白名单、目录沙箱、联网开关 |
| [Eval](../eval/) | SWE-bench / Terminal-Bench 测的是「模型 + Harness」，不是裸模型 |

公开榜上同一模型换 Harness，分数可以差一截。看 [Artificial Analysis Coding Agents](https://artificialanalysis.ai/?coding-agents=execution-time) 时，比的是整条链，不是裸模型的学校考试。为什么难评，见 [Eval](../eval/)。
