...
>
> 它实现的是 [循环与图工程](../../loop-graph/) 里的**执行图**（下一步跑谁，见 [execution-graph.md](../../loop-graph/execution-graph.md)）。**上下文图**（系统知道什么：客户 / 合同 / 政策）不在本目录——checkpoint 能续跑，并不定义「谁是同一个客户」。
>
> 本目录放在 [case-studies](../README.md) 下：它不是「一个造出来的系统」，而是**一个框架的可跑实操**（教程 + demo），与 commerce-agents / pi / trpc-agent-go 并列，用来对照「框架给你什么、不给你什么」。
...
- 循环与图工程（执行图 vs 上下文图）：[../../loop-graph/](../../loop-graph/) · [execution-graph.md](../../loop-graph/execution-graph.md)
- 同仓库 RAG 全景：[../../../knowledge/rag/rag-types.md](../../../knowledge/rag/rag-types.md)、[../../../knowledge/rag/rag-mvp.md](../../../knowledge/rag/rag-mvp.md)
