...
| 专题 | 对应 |
|---|---|
| [agent](../) | 这是 L2/L3 级（受限循环 + 验证门）的工业范本，人审=HITL |
| [guardrails](../../reliability/guardrails/) | 输入围栏、工具门禁、输出校验、人审——四个全占 |
| [agent-skills](../agent-skills/) | 十个 flow = 十个 `SKILL.md`，是 Skills 的电商落地 |
| [memory](../../knowledge/memory/) | 记忆写过滤、三类、保留/删除、只从最后一轮文本提取 |
| [context-engineering](../../knowledge/context-engineering/) | 围栏 + 缓存断点 + 历史压缩（`compact_history_above_tokens`） |
| [eval](../../reliability/eval/) | 仓库自带跨包测试 + `verify_all.py`；换模型/关审批要重跑 eval |
...
- 相邻专题：[agent](../)、[guardrails](../../reliability/guardrails/)、[agent-skills](../agent-skills/)、[harness](../harness/)
