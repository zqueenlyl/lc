...
7. **评估 + 可观测**：Eval sets + Metrics + OpenTelemetry + Langfuse 示例，对应 [eval](../eval/)。
...
| [mcp](../mcp/) | MCP 工具接入 |
| [a2a](../a2a/) | A2A server |
| [memory](../memory/) | `memory` 包（长期记忆、个性化） |
| [rag](../rag/) | `knowledge` 包（RAG 检索） |
| [agent-skills](../agent-skills/) | `skill` + `evolution`（技能 + 自我进化） |
| [eval](../eval/) | `evaluation` 包 + OpenTelemetry |
...
3. 生产先接 OpenTelemetry，把 trace 从第一天打起来（和 [agent 专题](../agent/) 的落地建议一致）。
...
- 相邻专题：[langgraph](../langgraph/)、[agent](../agent/)、[mcp](../mcp/)、[a2a](../a2a/)、[eval](../eval/)
