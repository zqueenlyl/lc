...
**操作向 notebook**（纯标准库；不替代上面五篇手册，把手册里必须算一遍的账跑成可改参数的代码。关卡地图见 [环节00](./环节00-总揽与环节导航.md)）：

| 环节 | Notebook | 验证什么 |
|---|---|---|
| 00 | [总揽与环节导航](./环节00-总揽与环节导航.md) | 与原理环节 10/11 的分工；01–04 怎么选 |
| 01 | [带宽天花板与量化档位](./环节01-带宽天花板与量化档位演示.ipynb) | `tok/s ≈ BW/W`；`t ≈ 1.6 + W/261`；量化只加速 decode |
| 02 | [KV Cache 才是内存主体](./环节02-KV-Cache才是内存主体演示.ipynb) | 112 KiB/token；522MB 权重为何驻留 5.6GB |
| 03 | [OpenAI 兼容探针与 TTFT 测法](./环节03-OpenAI兼容探针与TTFT测法演示.ipynb) | 只换 `base_url`；SSE 三字段；nonce；可选本机探测 |
| 04 | [五个不可比陷阱](./环节04-五个不可比陷阱演示.ipynb) | prefill/decode、ctx、同模型≠同精度、冷热、版本 |

> **未加链接 = 尚未创建**：新增（还没写完的）文档，在上表只写文件名（行内代码），不做 Markdown 链接，否则点击时会报「无法解析不存在的文件」；写完后再改回相对链接。**本目录当前 5 篇手册 + 环节 00 + notebook 01–04 均已创建。**
>
