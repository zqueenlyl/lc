...
运行时 Host 只先读 **短描述 / frontmatter** 做路由；命中后再把正文（和需要的 reference）塞进上下文。这是 [Context Engineering](../context-engineering/) 的一种具体做法：**按需加载程序记忆**。
...
- **与记忆分层**：Skill = 程序记忆（怎么做）；[Memory](../memory/) = 情景 / 语义（做过什么、用户是谁）。
...
| [Memory](../memory/) | 程序记忆 vs 情景记忆 |
| [Context Engineering](../context-engineering/) | Skill 是受控的上下文模块 |
| [Eval](../eval/) | 用「是否遵守 skill 硬规则」做回归 |
...
- 对比：[context-engineering](../context-engineering/)、[mcp](../mcp/)、[memory](../memory/)
