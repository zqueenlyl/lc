...
关键事实边界：模型 ID 均为 **2026-09-11 检索快照**，迭代极快，上线请以 `GET /v1/models` 或官方模型页校准。与 [`ai/landscape.md`](../ai/landscape.md) 有明确分工——**landscape 是产业扫盲层（不记版本号），本目录是工程可操作层（模型 ID / 端点 / 字段）**，两层互链不重复。MiniMax、火山方舟官方文档为 JS 单页应用，正文未能抓取，相关细节在文中标注为「未核实」。
